...
window.renderKasirMenu = () => {
  ...
  state.cachedProducts.forEach(p => {
    ...
    kasirMenuList.innerHTML += `
      <div class="kasir-item-card" data-kategori="${p.kategori || 'Umum'}" style="${habis ? 'opacity:.45;pointer-events:none;' : ''}" onclick="window.addToKasirCart('${p.id}', \`${p.nama}-${p.isi}pcs\`, ${p.harga}, ${stok})">
...
window.addToKasirCart = (id, nama, harga, stok) => {
  ...
  else { state.kasirCart[id] = { id, nama, harga, qty: 1, stok: stokMax }; }
  updateKasirCartUI();
};
